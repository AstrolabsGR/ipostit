...
function initTowhub() {
    ...
    //custom-astro
    if ($(".listing-inner-slider").length > 0) {
        var lis = new Swiper(".listing-inner-slider", {
            preloadImages: false,
            loop: true,
            spaceBetween: 0,
            slidesPerView: 1,
...
//click -----------------
    ...
    $(".clear-singleinput-1").on("click", function (e) {
        $(this).parents(".clact").find("input").val('');
    });
...
// open hours -----------------
    if ($(".opening-hours").length > 0) {
        ...
        var todayClass = weekday[d.getDay()];
        var todayElement = document.getElementsByClassName(todayClass)[0];
    
        if (todayElement) {
            todayElement.classList.add("todaysDay");
        }
    }
